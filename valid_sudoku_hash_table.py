...
def isValidSudoku(board: List[List[str]]) -> bool:
    ...
    for type in group_types:
        for number in range(9):
            ...
            group = getGroup(board, type, number)
            for numb in group:
                if chunk.get(numb):
                    return False
                elif numb != ".":
...
def getGroup(board: List[List[str]], type: str, number: int) -> List:
    group = []
    if type == "row":
        group = board[number]
    elif type == "column":
        for i in range(9):
            group.append(board[i][number])
    elif type == "square":
        starting_row = number // 3
        starting_column = number % 3
